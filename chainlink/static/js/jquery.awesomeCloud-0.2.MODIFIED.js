...
( function( $ ) {
	...
	AwesomeCloud.prototype = {
		create: function() {
            ...
            console.log($this.wordRectangles);
			$this.allDone( ctxID );
			return true;
		},
		...
		createCanvas : function ( options ) {
            ...
            //matt s changes to add eventHandler
            canvasDOM.addEventListener('click',function(e){
                alert("clickage");
                //find the rectangles that collide with mouse position
                var rect = AwesomeCloud.prototype.collides($this.wordRectangles, e.offsetX, e.offsetY);
                if (rect){
                    //create an event for other jQuery elements to listen to
                    alert("creating event");
                    var e = $.Event("cloudEvent",{'keyword':rect.word});
                    $("#" + canvasID ).trigger(e);
					...
					function ( gxy ) {
						if ( $this.canFitText( gxy[ 0 ], gxy[ 1 ], gw, gh ) ) {
							if ( rotate ) {
                                ...
                                var left = gxy[ 0 ] * $this.settings.gridSize + ( gw * $this.settings.gridSize - w ) / 2;
                                var top =  gxy[ 1 ] * $this.settings.gridSize + ( gh * $this.settings.gridSize - h ) / 2;
                                //save the relevant data for later for collision detections
                                $this.wordRectangles.push({x:left,
                                                           y:top,
                                                           h:h* (.8), //scaling for clicks later
                                                           w:w,
                                                           word:word});

                                $this.ctx.fillText( word,left ,top);
							}
        ...
        collides : function(rects, mouseX, mouseY){
            //given a set of rectangles, determine if the mouse is in one
            var collidedRects = [];
            var ret = [];
            for (var i in rects){
                //build the box
                ...
                //check to see if we are in the box
                if(right >= mouseX && left <= mouseX && bottom >= mouseY && top <= mouseY ){

                    collidedRects.push(rects[i]);
                }
            }

            collidedRects.sort(function(a,b){
               return a.w * a.h - b.w * b.h
            });
            //assume they want the smallest box that collides
            if(collidedRects.length != 0){
                ret = collidedRects[0]
            }

            return collidedRects[0];
        },
		...
		isGridEmpty : function ( imgData, x, y, w, h, skipDiffChannel ) {
			...
			}//htmlStr += "<span data-weight='" + words[key] + "' style='display: none;'>" +key + "</span>";
			return true;
		},
